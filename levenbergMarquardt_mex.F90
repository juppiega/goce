...
function structToDerived_TexAndMajor(matlabStruct, typeName) result(D)
    ...
    allocate(D%P20(N), D%P21(N), D%P22(N), D%mP20(N))
    allocate(D%P30(N), D%P31(N), D%P32(N), D%P33(N), D%mP30(N), D%mP31(N))
    allocate(D%P40(N), D%P43(N), D%P44(N), D%mP40(N))
    allocate(D%P50(N), D%P51(N), D%P52(N), D%P53(N), D%P54(N),D%mP50(N),D%mP51(N))
...
subroutine deallocateStruct(D, typename)
    ...
    deallocate(D%P20, D%P21, D%P22, D%mP20)
    deallocate(D%P30, D%P31, D%P32, D%P33, D%mP30, D%mP31)
    deallocate(D%P40, D%P43, D%P44, D%mP40)
    deallocate(D%P50, D%P51, D%P52, D%P53, D%P54, D%mP50, D%mP51)
