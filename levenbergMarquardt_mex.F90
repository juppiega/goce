...
function G_storm(a, S)
    ...
    type(dataStruct), intent(in) :: S
    real(kind = 8), intent(in) :: a(:)
    real(kind = 8), allocatable :: G_storm(:), Activity(:), mag_lat(:), mag_lon(:), mag_lst(:)
    integer :: k
    k = 0

    k = 0;

    Activity = a(k+1)*log(S%aeInt(:,1)) + a(k+2)*log(S%aeInt(:,2)) + a(k+3)*log(S%aeInt(:,3)) +&
               a(k+4)*log(S%aeInt(:,5)) + a(k+5)*log(S%aeInt(:,6)) + a(k+6)*log(S%aeInt(:,7));
    k = k + 6;
...
subroutine mexFunction(nlhs, plhs, nrhs, prhs)
    ...
    call lmSolve(modelMinimizationFunction, initGuess, paramsToFit, tolX, tolFun, tolOpt, lambda0, maxFuncEvals, maxIter, &
