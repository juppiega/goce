...
function structToDerived_TexAndMajor(matlabStruct, typeName) result(D)
    ...
    call mxCopyPtrToReal8(mxGetPr(mxGetField(matlabStruct, 1, 'mP22')), D%mP22, N)
...
function G_major(a, S, numBiases)
    ...
    geomagnetic = geomParametrization(S, a(k+1:k+6), S%aeInt(:,1)) +&
                ...
                geomParametrization(S, a(k+31:k+36), S%aeInt(:,5)*S%aeInt(:,6)/1000);
...
function G_storm(a, S)
    ...
    real(kind = 8), allocatable, dimension(:) :: order_0, order_1, order_2, lagHours
    real(kind = 8) :: an, san
    integer :: i
...
!    k = k + 11;
    ...
    lagHours = mod(mod(a(k+1),24D0)*(1 + a(k+2)*abs(S%mP10)), 23D0) + 1;
...
function evalTex(S, coeff)
    ...
    evalTex = coeff(1) * (1 + clamp(dble(-0.9), G_majorTex(coeff, S, 0), dble(9.0))) 
...
function evalMajorSpecies(S, coeff, numBiases)
    ...
    evalMajorSpecies = exp(coeff(1) + G_majorTex(coeff, S, numBiases));
...
function computeRho(T0, dT0, Tex, Z, OlbDens, N2lbDens, HelbDens, ArlbDens, O2lbDens) result(rho)
    ...
    integer(kind = 4) :: mexCallMATLAB, mexStat
...
subroutine mexFunction(nlhs, plhs, nrhs, prhs)
    ...
    real(kind = 8) :: tolX, tolFun, tolOpt, lambda0, minLambda, firstOrderOpt
    ...
    if(nrhs /= 17) then
        call mexErrMsgTxt ('Levenberg-Marquardt: 16 inputs required!')
    ...
    if (mxIsDouble(prhs(8)) .eq. 0 .or. mxIsDouble(prhs(9)) .eq. 0) then
        call mexErrMsgTxt ('Levenberg-Marquardt: inputs 8 and 9 must be real arrays!')
    end if
    ...
    N = max(mxGetM(prhs(12)), mxGetN(prhs(12)))
    allocate(paramReal(N))
    call mxCopyPtrToReal8(mxGetPr(prhs(12)), paramReal, N)
    allocate(paramsToFit(N))
    paramsToFit = nint(paramReal, 4)

    call mxCopyPtrToReal8(mxGetPr(prhs(13)), tolX, 1)
    call mxCopyPtrToReal8(mxGetPr(prhs(14)), tolFun, 1)
    call mxCopyPtrToReal8(mxGetPr(prhs(15)), tolOpt, 1)
    call mxCopyPtrToReal8(mxGetPr(prhs(16)), lambda0, 1)
    call mxCopyPtrToReal8(mxGetPr(prhs(17)), minLambda, 1)
    ...
    deallocate(weights, initGuess, paramReal, paramsToFit, dTCoeffs, T0Coeffs)
