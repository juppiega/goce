...
if fitSimultaneously || fitBaseAgain
    ...
    disp('Calling LM solver')
    clear mex;
    rhoStruct.data(rhoStruct.goce) = rhoStruct.data(rhoStruct.goce) * 0.97;
...
ind = datenum('2005-05-13') < rhoStruct.timestamps & rhoStruct.timestamps < datenum('2005-05-20');
ind(datenum('2004-11-05') < rhoStruct.timestamps & rhoStruct.timestamps < datenum('2004-11-15')) = true;
ind(datenum('2003-10-27') < rhoStruct.timestamps & rhoStruct.timestamps < datenum('2003-11-03')) = true;
ind = find(ind); ind = ind + TempAndSpectrometerLen;
ind = ismember(1:length(weights), ind);
w = sum(weights(~ind)) / sum(weights(ind));
weights(ind) = 0.05*w * weights(ind);
