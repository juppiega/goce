...
if length(goceStruct.data) > 0; [goceBI, goceEI, goceCI] = findStormsForSat(goceStruct, dstOrAE, threshold,0,1); else goceBI = []; end
if length(champStruct.data) > 0; [champBI, champEI, champCI] = findStormsForSat(champStruct, dstOrAE, threshold,0,1); else champBI = []; end
if length(graceStruct.data) > 0; [graceBI, graceEI, graceCI] = findStormsForSat(graceStruct, dstOrAE, threshold,0,1); else graceBI = []; end
if length(swarmStruct.data) > 0; [swarmBI, swarmEI, swarmCI] = findStormsForSat(swarmStruct, dstOrAE, threshold,0,1); else swarmBI = []; end
