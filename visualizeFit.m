...
if exist('msisDtmComparison.mat', 'file')
    ...
    [~, msisRho, dtmRho, jbRho] = computeComparisonData(originalRhoStruct, coeffStruct, numBiasesStruct);
    ...
    save('msisDtmComparison.mat', 'msisRho', 'dtmRho', 'jbRho', 'msis_370','dtm_370', 'msis_270','dtm_270', 'msis_470','dtm_470')
end
...
ind = ~removeInd;
modelStruct = struct('il', ilRho(ind), 'msis', msisRho(ind), 'dtm', dtmRho(ind), 'jb', jbRho);
...
% % % % % 
      ...
      %plotStormFig(originalRhoStruct, modelStruct, '2005-05-13', '2005-05-20', 'GRACE', coeffStruct, numBiasesStruct, saveFolder,fullscreenFigs);
      %plotStormFig(originalRhoStruct, modelStruct, '2005-05-13', '2005-05-20', 'CHAMP', coeffStruct, numBiasesStruct, saveFolder,fullscreenFigs);
...
elseif strcmpi(paramName, 'N2_O')
    ...
    [~,~,dtm_O] = computeDtm(S);
    param = il_N2./il_O;
    msisParam = msis_N2./msis_O;
    dtmParam = dtm_N2./dtm_O;
...
function model = fixBiasBySatellite(model, rhoStruct)
...
rawOM = zeros(length(stormBeginInd),4);
rawRMS = zeros(length(stormBeginInd),4);
OACorr = zeros(length(stormBeginInd),4);
OAOM = zeros(length(stormBeginInd),4);
OARMS = zeros(length(stormBeginInd),4);
...
for i = 1:length(stormBeginInd)
    ...
    firstDay = timestamps < timestamps(1) + 2;
...
%     end
    ...
    msisOrbAver = computeOrbitAverage(msisRho, lat, timestamps);
    dtmOrbAver = computeOrbitAverage(dtmRho, lat, timestamps);
    jbOrbAver = computeOrbitAverage(jbRho, lat, timestamps);
    ...
    rawCorr(i,4) = corr(jbRho, measuredRho);
    ...
    OARMS(i,4) = rms(measuredOrbAver ./ jbOrbAver-1);
...
outputCell(2:end, 6:9) = num2cell(rawCorr);
outputCell(2:end, 10:13) = num2cell(rawOM);
outputCell(2:end, 14:17) = num2cell(rawRMS);
outputCell(2:end, 18:21) = num2cell(OACorr);
outputCell(2:end, 22:25) = num2cell(OAOM);
outputCell(2:end, 26:29) = num2cell(OARMS);

outputCell(1,1:end) = {'Begin', 'End', 'Mean F10.7', 'Min Dst', 'SatInfo (0=GO:1=CH:2=GR:3=SW)',...
                        'Corr. IL','Corr. MSIS','Corr. DTM','Corr. JB',...
                        'O/M IL','O/M MSIS','O/M DTM','O/M JB',...
                        'RMS IL','RMS MSIS','RMS DTM','RMS JB', ...
                        'OA Corr. IL', 'OA Corr. MSIS', 'OA Corr. DTM','OA Corr. JB',...
                        'OA O/M IL', 'OA O/M MSIS', 'OA O/M DTM','OA O/M JB',...
                        'OA RMS IL', 'OA RMS MSIS','OA RMS DTM','OA RMS JB'};
...
fprintf(outputFile, 'JB2008 O/C: %f \n\n', mean(rhoStruct.data./jbRho));
...
fprintf(outputFile, 'JB2008 RMS: %f \n\n', rms(rhoStruct.data./jbRho-1));
...
fprintf(outputFile, 'JB2008 STD: %f \n\n', std(rhoStruct.data./jbRho));
...
fprintf(outputFile, 'JB2008 CORR: %f \n\n', corr(rhoStruct.data,jbRho));
