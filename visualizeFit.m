...
%plotSurfs(500, 200, 200, 180, 'Rho', TexCoeff, OCoeff, N2Coeff, HeCoeff);

if exist('comparisonRho.mat', 'file')
    load comparisonRho.mat
else
    [ilRho, msisRho, dtmRho] = computeComparisonData(rhoStruct, TexCoeff, OCoeff, N2Coeff, HeCoeff);
...
function [ilRho, msisRho, dtmRho] = computeComparisonData(rhoStruct, TexCoeff, OCoeff, N2Coeff, HeCoeff)

rhoStruct = computeVariablesForFit(rhoStruct);

[Tex, dT0, T0] = findTemps(rhoStruct, TexCoeff);
OlbDens = evalSpecies(rhoStruct, OCoeff);
N2lbDens = evalSpecies(rhoStruct, N2Coeff);
HelbDens = evalSpecies(rhoStruct, HeCoeff);
...
for i = 1:N
    [He(i), O(i), N2(i),~,~,rho(i),~,~,~,Tex(i)] = nrlmsise_mex(S.doy(i),43200,S.altitude(i),S.latitude(i),S.longitude(i),S.solarTime(i),...
